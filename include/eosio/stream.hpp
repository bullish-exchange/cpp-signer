...
inline std::string convert_stream_error(stream_error e) {
   switch (e) {
      // clang-format off
...
template<typename T>
bool has_bitwise_serialization() {
   ...
   || std::is_same<T, __int128>::value || std::is_same<T, unsigned __int128>::value
      ...
      static_assert(!std::is_convertible<T, typename std::underlying_type<T>::value>::value, "Serializing unscoped enum");
      return true;
   } else {
      return false;
   }
...
struct input_stream {
   ...
   explicit input_stream(const std::string& v) : pos{ v.data() }, end{ v.data() + v.size() } {}
   ...
   auto get_pos() -> decltype(pos) const { return pos; }
